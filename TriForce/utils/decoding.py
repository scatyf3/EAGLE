...
def TriForce(
    ...
    torch.cuda.synchronize()
    prefill_time = time.time() - prefill_t0
    ...
    while n < max_len:
        ...
        if use_draft:
            # Two-stage speculative decoding: llama-68m draft + retrieval 7b verify
            pred_token_idx = next_token
            verify_tokens, speculation_probs, acc_rate_middle, s1_acc, s1_prop = Middle_Spec(
                pred_token_idx, graph_engine, gamma, False, tokenizer, use_draft=True
            )
            if acc_rate_middle is not None:
                acc_rate_middle_list.append(acc_rate_middle)
            stage1_accepted_total += s1_acc
            stage1_proposed_total += s1_prop
            ...
            # Keep subset-KV middle verify, but remove 68m proposer.
            # Proposal is sampled directly from subset-KV verify distribution.
            pred_token_idx = next_token
            verify_tokens, speculation_probs, acc_rate_middle, s1_acc, s1_prop = Middle_Spec(
                pred_token_idx, graph_engine, gamma, False, tokenizer, use_draft=False
            )
            if acc_rate_middle is not None:
                acc_rate_middle_list.append(acc_rate_middle)
            stage1_accepted_total += s1_acc
        ...
        # update cache for 68m (only if using draft)
        if use_draft:
    ...
    stage1_acceptance_rate = (stage1_accepted_total / stage1_proposed_total) if stage1_proposed_total > 0 else None
    stage2_acceptance_rate = acceptance_rate
    ...
    if file_path is not None:
        ...
        acc_middle_mean = np.array(acc_rate_middle_list).mean() if acc_rate_middle_list else 0.0
        entry = f"{graph_engine.engine.model.config._name_or_path},{acceptance_rate},{n / (time2 - time1)},{avg_tokens},{input_ids.shape[1]},{n},{dataset},{acc_middle_mean},{(time2 - time1)/n}\n"
    ...
    decode_time = time2 - time1
    decode_tps = n / decode_time if decode_time > 0 else 0.0
    stage_stats = {
        "stage1_acceptance_rate": stage1_acceptance_rate,
        "stage1_accepted_len": stage1_accepted_total,
        "stage1_proposed_len": stage1_proposed_total,
        "stage2_acceptance_rate": stage2_acceptance_rate,
        "stage2_accepted_len": accepted_count,
        "stage2_proposed_len": draft_count,
    }
    if return_timing:
        return acceptance_rate, decode_tps, prefill_time, decode_time, prefill_time + decode_time, stage_stats
    return acceptance_rate, decode_tps

@torch.inference_mode()
def Middle_Spec(next_token, graph_engine, gamma, verbose, tokenizer, use_draft=True):
    ...
    while n < gamma:
        if use_draft:
            ...
            # No 68m proposer: proposal comes from subset-KV verify distribution directly.
            verify_prob = graph_engine.graph_verify(input_ids=verify_tokens, position_ids=position_ids)
            speculation_prob = verify_prob[n]
            pred_token_idx = sample(speculation_prob)
            token_idx = pred_token_idx.item()
            draft_count += 1

            return_speculation_probs.append(speculation_prob)
            return_generated_ids.append(token_idx)
            if verbose:
                spec_stream(pred_token_idx, tokenizer, 'blue')
    ...
    if not use_draft:
        ...
        acceptance_rate = accepted_count / draft_count
    return return_generated_ids, return_speculation_probs, acceptance_rate, accepted_count, draft_count
