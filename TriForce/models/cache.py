...
class RetrievalCache(Cache):
    ...
    def init_graph_cache(self, kv_cache, query_states, layer_idx):
        ...
        # When prefill is short, available chunks can be smaller than configured select_sets.
        eff_sets = min(self.select_sets, self.chunks)

        # (bsz, 32, select_sets) --> (bsz, select_sets, 32)
        _, topk_idx_rest = torch.topk(chunk_attn[:, :, 1:], k=eff_sets-1, dim=-1)
        topk_idx_rest += 1
        ...
        topk_idx = torch.cat([topk_idx_first, topk_idx_rest], dim=-1)  # (bsz, 32, eff_sets)
        ...
        retrieved_tokens = eff_sets * self.chunk_size
        self.key_cache[layer_idx][:,:retrieved_tokens] = result_tensor.permute(0, 1, 3, 2, 4).reshape(1, retrieved_tokens, self.num_heads, self.head_dim).clone()
        ...
        self.value_cache[layer_idx][:,:retrieved_tokens] = result_tensor.permute(0, 1, 3, 2, 4).reshape(1, retrieved_tokens, self.num_heads, self.head_dim).clone()

        if layer_idx == self.layers-1:
            self.init_graph = True

    def update_graph_cache(self, kv_cache=None):
        live_tokens = kv_cache.seq_len - self.prefill
        if live_tokens <= 0:
            return
        copy_tokens = min(live_tokens, self.max_budget)
        src_start = kv_cache.seq_len - copy_tokens
        dst_start = self.max_budget - copy_tokens
        self.value_cache[:, :, dst_start:self.max_budget] = kv_cache.value_cache[:, :, src_start:kv_cache.seq_len].clone()
        self.key_cache[:, :, dst_start:self.max_budget] = kv_cache.key_cache[:, :, src_start:kv_cache.seq_len].clone()
    ...
    def update_graph_cache_retrieval(self, kv_cache, query_states, layer_idx):
        ...
        src_start = kv_cache.seq_len - copy_tokens
        dst_start = self.max_budget - copy_tokens
        self.value_cache[layer_idx, :, dst_start:self.max_budget] = kv_cache.value_cache[layer_idx, :, src_start:kv_cache.seq_len].clone()
        self.key_cache[layer_idx, :, dst_start:self.max_budget] = kv_cache.key_cache[layer_idx, :, src_start:kv_cache.seq_len].clone()
...
class StreamingLLMEvictionCache(Cache):
    ...
    def spec_update(self, new_k_cache :torch.Tensor, new_v_cache :torch.Tensor, layer_idx :int, gamma_offset=0):
        ...
        # Some runtimes may emit a speculative segment longer than (gamma + 3).
        # Keep the most recent segment by shifting the write window left to avoid overflow.
        incoming = int(new_k_cache.shape[-3])
        tail_start = self.real_budget - self.gamma - 3
        start = tail_start
        end = start + incoming
        if end > self.real_budget:
            start = max(0, self.real_budget - incoming)
            end = self.real_budget
